        ...
        <PageWrapper>
          <div className="text-center py-16">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', damping: 18 }}>
              ...
              <p className="text-sm" style={{ color: '#555' }}>Start je eerste training om je geschiedenis bij te houden</p>
            </motion.div>
          </div>
        </PageWrapper>
      </>
        ...
        >
          {[
            ...
            >
              ...
              <p className="text-2xl font-heading tracking-wider m-0" style={{ color: '#FAFAFA' }}>{value}</p>
              <p className="text-[10px] m-0 mt-0.5 uppercase tracking-wider" style={{ color: '#444' }}>{label}</p>
            </div>
          ))}
        </motion.div>
        ...
        {thisWeek.length > 0 && (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-semibold uppercase tracking-widest m-0" style={{ color: '#FF5500', letterSpacing: '0.1em' }}>
          ...
          {grouped.map(({ month, sessions: ms }, gi) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <h3 className="text-sm font-heading tracking-widest m-0 uppercase" style={{ color: '#FF5500', letterSpacing: '0.15em' }}>
                  {month}
                </h3>
                <div className="flex-1 h-px" style={{ background: 'linear-gradient(to right, #1C1C1C, transparent)' }} />
                <span className="text-xs" style={{ color: '#333' }}>{ms.length}×</span>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{ background: '#060606', borderColor: grad.dot, boxShadow: `0 0 8px ${grad.glow}` }}
                        >
                              ...
                              {session.exercises.map(se => {
                                ...
                                return (
                                  <div key={se.exerciseId} className="flex items-center justify-between">
                                    <span className="text-xs truncate flex-1 mr-2" style={{ color: '#666' }}>
                            ...
                            {isExpanded && (
                              ...
                              >
                                <div className="px-4 pb-4 pt-3 space-y-3" style={{ borderTop: '1px solid #1C1C1C' }}>
                                  {session.exercises.map(se => {
                                    ...
                                    return (
                                      <div key={se.exerciseId}>
                                        <p className="text-xs font-semibold mb-1.5 m-0" style={{ color: '#888' }}>
                                          {exName(exercise) || se.exerciseId}
                                        </p>
                                  ...
                                  {session.notes && (
                                    <p className="text-xs italic mt-2 m-0" style={{ color: '#444' }}>"{session.notes}"</p>
                                  )}
                                </div>
                              </motion.div>
                            )}
